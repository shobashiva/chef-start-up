...
cmd = 'rsync -rv --exclude=.git --exclude=.gitignore --exclude=start-chef --exclude=README.md ' + '../'  + ' ../tmp_' + root_repo_name
`#{cmd}`
puts cmd
...
cmd = 'berks cookbook ../chef-' + root_repo_name
`#{cmd}`
...
while !valid_chef_git_repo do
	...
	cmd = 'env GIT_PROXY_COMMADN=myproxy.sh GIT_TRACE=1 git ls-remote ' + try_chef_git_repo
	if `#{cmd}`
		chef_git_repo = try_chef_git_repo.strip
		valid_chef_git_repo = true
	else
		puts try_chef_git_repo
		puts "A valid git repo was not provided"
	end
end
...
if !File.exists? metadata_location
    ...
    template = File.read("templates/metadata.erb")
...
if !File.exists? system_recipe_location
    ...
    template = File.read("templates/system.erb")
...
puts "Enter a name for your database: "
dbname = gets.chomp
...
if !File.exists? data_recipe_location
    ...
    template = File.read("templates/data.erb")
...
if !File.exists? django_recipe_location
    ...
    template = File.read("templates/django.erb")
...
puts "Enter the path to your settings file, relative to your repository's root folder (it should begin with " + root_repo_name + "/" + root_repo_name + "): "
settings_file = gets.chomp

puts "Enter the path to your settings-dist file, relative to your repository's root folder (it should begin with " + root_repo_name + "/" + root_repo_name + "): "
settings_dist = gets.chomp
...
if !File.exists? code_recipe_location
    ...
    template = File.read("templates/code.erb")
...
puts "Enter the version of python the project uses (likely python2.7 or python3): "
python_version = gets.chomp
...
if !File.exists? server_recipe_location
    ...
    template = File.read("templates/server.erb")
...
if !File.exists? start_vm_location
    ...
    template = File.read("templates/start_vm.erb")
